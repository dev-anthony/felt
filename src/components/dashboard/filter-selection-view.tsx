...
//   return (
//     /* Changed to full width/height centering without stretching content to edges */
//     <div className="w-full min-h-[560px] flex flex-col justify-center items-center p-6 bg-transparent">
      
//       {/* Bounding width layout box to keep elements beautifully stacked together */}
//       <div className="w-full max-w-md mx-auto space-y-5 flex flex-col">
...
//         {/* Horizontal Filter Carousel Row */}
...
export function FilterSelectionView({ trackTitle, onProceed }: FilterSelectionViewProps) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-between p-6">
      <div className="flex-1 flex flex-col justify-center w-full max-w-md mx-auto space-y-5">
        ...
        {/* Preview frame — capped height so it never pushes content off on small screens */}
        <div className="w-full border border-border/40 relative flex flex-col items-center justify-center overflow-hidden bg-background" style={{ aspectRatio: "16/9", maxHeight: "180px" }}>
          <div className={`absolute inset-0 opacity-40 blur-xl scale-110 transition-all duration-500 ${selectedFilter.style}`} />
          <div className="relative z-10 text-center space-y-1.5 p-3">
            <div className="size-9 mx-auto rounded-none border border-foreground/20 flex items-center justify-center bg-[#0d0d0d] shadow-2xl">
              <ImageIcon className="size-4 text-muted-foreground stroke-[1.25px]" />
            </div>
            <p className="font-display italic text-base text-foreground truncate max-w-[260px]">{trackTitle}</p>
            <span className="font-mono text-[9px] tracking-widest uppercase px-2 py-0.5 bg-foreground text-background font-bold">
              Active: {selectedFilter.name}
            </span>
          </div>
          <div className="absolute bottom-2 left-3 right-3 flex items-center justify-between pointer-events-none gap-2">
            <span className="font-mono text-[8px] text-muted-foreground/60 tracking-wider shrink-0">DESCRIPTOR:</span>
            <span className="font-mono text-[8px] text-accent/80 tracking-wider uppercase truncate">{selectedFilter.label}</span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {paginatedFilters.map((filter) => {
              ...
              return (
                ...
                >
                  <div className={`aspect-square w-full relative overflow-hidden mb-1 border border-border/20 ${filter.style}`}>
                    {isCurrent && (
                      <div className="absolute inset-0 bg-background/20 backdrop-blur-xs flex items-center justify-center">
                        <Check className="size-4 text-foreground stroke-[3px]" />
                      </div>
        ...
        <div className="pt-4 flex items-center justify-end gap-3 border-t border-border/20">
          ...
          <Button
            type="button"
            onClick={() => onProceed(selectedFilter.id)}
            className="font-mono text-[10px] uppercase tracking-widest rounded-none h-10 px-6 bg-foreground text-background hover:bg-foreground/90 w-full sm:w-auto"
          >
            Looks Right → Proceed
          </Button>
        </div>
