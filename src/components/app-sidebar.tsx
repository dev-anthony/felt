...
export function AppSidebar() {
  ...
  // Navigation schema built exactly for FELT Studio Workspace
  ...
  return (
    ...
    >
      ...
      <SidebarHeader className="h-20 flex flex-row items-center justify-between px-4 border-b border-border/40 bg-[#080808]">
        {state === "expanded" && (
          <span className="font-display italic text-2xl tracking-tight select-none reveal">
            FELT
          </span>
        )}
        
        {/* Responsive Desktop Structural Toggle Button */}
        {!isMobile && (
          <button
            onClick={toggleSidebar}
            className={
              "p-1.5 hover:bg-foreground/5 text-muted-foreground hover:text-foreground transition-all duration-300 border border-border/40 bg-transparent rounded-none ml-auto"
            }
            style={{
