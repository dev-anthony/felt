      ...
      <div className="md:hidden fixed bottom-6 right-6 z-50">
        ...
        >
          <div className='flex flex-col gap-3'>
            {context === 'landing' ? (
              <>
                <a href="#" onClick={handleLinkClick} className={linkClass} title="Home"><Home className="w-5 h-5"/></a>
                <a href="#summary" onClick={handleLinkClick} className={linkClass} title="Summary"><Sparkles className="w-5 h-5"/></a>
                <a href="#features" onClick={handleLinkClick} className={linkClass} title="Features"><Layers className="w-5 h-5"/></a>
                <a href="#pricing" onClick={handleLinkClick} className={linkClass} title="Pricing"><Ticket className="w-5 h-5"/></a>
                <button onClick={() => { handleLinkClick(); setAuthOpen(true); }} className={linkClass} title="Get Started">
                  <ArrowUpRight className="w-5 h-5"/>
                </button>
              </>
            ) : (
              <>
                <a href="/dashboard" onClick={handleLinkClick} className={linkClass} title="Dashboard Home"><LayoutDashboard className="w-5 h-5"/></a>
                <a href="/dashboard/archive" onClick={handleLinkClick} className={linkClass} title="Archive"><Archive className="w-5 h-5"/></a>
                <button onClick={handleLogout} className={`${linkClass} text-destructive hover:bg-destructive/15`} title="Logout">
        ...
        {isOpen && (
          <div onClick={toggleNav} className="fixed inset-0 bg-black/20 dark:bg-black/50 backdrop-blur-sm z-30 transition-opacity duration-300" />
        )}
      </div>
